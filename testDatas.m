...
% 4. Adım: Sistem Parametreleri ve Eşik Değerleri (Güncellenmiş)
std_noise = 0.2;         
impulse_th = 4.0;        
g_th = 0.8; 
...
ax_params          = struct('LTA_win_size', 500/dt_master, 'STA_win_size', 100/dt_master, 'alpha', 0.88, 'threshold', 1.5);
rear_wheel_params  = struct('LTA_win_size', 500/dt_master, 'STA_win_size', 100/dt_master, 'alpha', 0.88, 'threshold', 1.5);
front_wheel_params = struct('LTA_win_size', 500/dt_master, 'STA_win_size', 100/dt_master, 'alpha', 0.88, 'threshold', 1.5);
pitch_rate_toga_th = 0; 
ralt_max_range = 4800;

% 5. Adım: İlk Filtreleme (Kırpmadan ÖNCE yapılmalıdır)
...
% =========================================================================
% 6. Adım: VERİ KIRPMA (CROPPING) - 600,000. veriden sonrasını al
% =========================================================================
start_idx = 600000;
if start_idx < len
    keep_idx = start_idx : len;
    ...
    % Zaman eksenini ve uzunluğu kırpılmış veriye göre baştan kur
    t = (0 : length(ax) - 1)' * dt_master;
    len = length(t); 
end
% =========================================================================

% 7. Adım: Sentetik Wheel Speed Oluşturma (Artık sadece kırpılmış veri üzerinde çalışır)
dummy = zeros(size(ax));
dummy2 = zeros(size(ax));
for i = 1 : len
    [dummy(i), dummy2(i)] = spikeDetect(ax_params, ax(i), "AX");
    if i < 1000
        dummy(i) = 0 ;
        dummy2(i) = 0;
    end
...
front_wheel_delay = 0; %no delay 

front_wheel_speed = zeros(size(t));
rear_wheel_speed = zeros(size(t));


% Uncomment for whole simulation 
% if ~isempty(take_off_idx)
...
% else
%     front_wheel_speed(1 : min(landing_idx + front_wheel_delay, len)) = 50;
%     rear_wheel_speed(1 : landing_idx) = 50;
% end
% 
% if ~isempty(landing_idx) && landing_idx + front_wheel_delay < len
%     front_wheel_speed(landing_idx + front_wheel_delay : end) = 350;
% end
% if ~isempty(landing_idx)
%     rear_wheel_speed(landing_idx : end) = 350;
% end

%Uncomment for landing only
rear_wheel_speed(take_off_idx : end) = 350;
front_wheel_speed(take_off_idx + front_wheel_delay : end) = 350;
...
% Main Loop İçin Hafıza (Pre-allocation)
...
% title('Altitude Analysis (Data After Index 600,000)');
% ylabel('Altitude (ft)');
